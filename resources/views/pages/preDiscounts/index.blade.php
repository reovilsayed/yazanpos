                    ...
                    <div class="tab-content">
                        ...
                        <div class="tab-pane fade active show" id="list" role="tabpanel"
                            ...
                            <table class="list_table all custom-fade-in">
                                ...
                                <tbody>
                                    @foreach ($preDiscounts as $preDiscount)
                                        <tr>
                                            <td>
                                                {{ $preDiscount->title }}
                                            </td>
                                            ...
                                            <td>
                                                @if (auth()->user()->role->hasPermissionTo('edit pre-discount'))
                                                    <a href="{{ route('pre-discounts.edit', $preDiscount) }}"
                                                ...
                                                @if (auth()->user()->role->hasPermissionTo('delete pre-discount'))
                                                    <x-actions.delete :action="route('pre-discounts.destroy', $preDiscount)" />
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="">Create Discount</h5>
                    <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form action="{{ route('pre-discounts.store') }}" method="post">
                    @csrf
                    <div class="modal-body">
                        <x-form.input name="title" :value="old('title')" label="Label" id="title" required />
                        <x-form.input name="amount" :value="old('amount')" label="Discount Amount (%)" id="amount"
                            required />
                    </div>
